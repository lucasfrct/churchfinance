...
Tester::on ( "Connect::on ( );", function ( $assert ) {
	...
	Connect::off ( );
	$assert::ok (  $connect , "Instancia a Classe connect" );
}, 10 );

Tester::on ( "Connect::on ( )->query ( );", function ( $assert ) {
	$assert::ok ( Connect::on ( )->query ( "INSERT INTO crud.users ( name, email, emailSecurity, password ) VALUES ( 'Name', 'Email 1', 'email Sec', 'Pass' )" ), "Registrar insert em banco de dados." );
	Connect::off ( );
} );
...
Tester::on ( "Crud : create ", function ( $assert ) { 
	...
	$crud->run ( );
	$assert::ok ( Modeldata::on ( )->parseBool ( $crud->response ( ) ), "Testa se o crud interpreta o json com action create e insere no banco de dados." ); 
} );
...
Tester::on ( "Crud read", function ( $assert ) { 
	...
	$assert::ok ( $test, "Testa se o crud interpreta o json com action read e tras dados do banco de dados" ); 
} );
